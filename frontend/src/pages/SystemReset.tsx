...
import {
  ...
  Link2, FileSearch, Trash2, AlertTriangle, CheckCircle, X
} from 'lucide-react';
...
const RESET_CARDS: ResetCard[] = [
  ...
  },
  {
    ...
    description: 'Clears ALL of the above. Use after an Opera database restore to ensure clean state. No Opera data is affected.',
    icon: Trash2,
    tables: [],
    variant: 'danger',
  },
...
export function SystemReset() {
  ...
  const [result, setResult] = useState<{ action: string; total: number } | null>(null);
  ...
  const handleExecute = async (action: string) => {
    ...
    try {
      ...
      setResult({ action, total: data.total_deleted });
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <PageHeader icon={RotateCcw} title="System Reset" subtitle="Clear application data and caches" />
        <LoadingState message="Loading record counts..." />
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-5xl">
      <PageHeader icon={RotateCcw} title="System Reset" subtitle="Clear application data and caches. Opera transactions are never affected." />
      ...
      {result && (
        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-start gap-2">
          ...
          <div className="flex-1">
            <p className="text-sm text-green-800 font-medium">Reset complete</p>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-gray-200 pt-3" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="mt-3 flex items-center gap-2 p-2 bg-amber-50 border border-amber-200 rounded-md">
                    ...
                    <p className="text-xs text-amber-800">This cannot be undone. {count.toLocaleString()} record{count !== 1 ? 's' : ''} will be permanently deleted.</p>
                  </div>
