...
export function StatutoryAccounts() {
  ...
  const AccSectionHeader = ({ title, total, section, icon: Icon }: { title: string; total: number; section: string; icon: typeof FileText }) => (
    <tr
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader icon={Building} title="Statutory Accounts" subtitle={`UK GAAP format accounts for the year ended ${selectedYear}`}>
        <button
      ...
      {accountsQuery.isLoading ? (
        <LoadingState message="Loading statutory accounts..." />
      ) : data?.success === false ? (
        <Alert variant="error" title="Error loading accounts">{data?.error}</Alert>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <div className="flex items-center gap-2 mb-4 pb-3 border-b">
              ...
              <h3 className="text-base font-bold text-gray-900">Profit and Loss Account</h3>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody>
                  {/* Turnover */}
                  <AccSectionHeader title="Turnover" total={pnl?.turnover?.total || 0} section="turnover" icon={TrendingUp} />
                  ...
                  {/* Cost of Sales */}
                  <AccSectionHeader title="Cost of Sales" total={-(pnl?.cost_of_sales?.total || 0)} section="cost_of_sales" icon={TrendingDown} />
                  ...
                  <AccSectionHeader title="Administrative Expenses" total={-(pnl?.administrative_expenses?.total || 0)} section="admin" icon={FileText} />
          ...
          <Card>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody>
                  {/* Fixed Assets */}
                  <AccSectionHeader title="Fixed Assets" total={bs?.fixed_assets?.total || 0} section="fixed_assets" icon={Building} />
                  {expandedSections.has('fixed_assets') && bs?.fixed_assets?.items?.map((item, idx) => (
                    <LineItem key={idx} code={item.code} description={item.description} value={item.value} />
                  ))}

                  {/* Current Assets */}
                  <AccSectionHeader title="Current Assets" total={bs?.current_assets?.total || 0} section="current_assets" icon={Wallet} />
                  {expandedSections.has('current_assets') && bs?.current_assets?.items?.map((item, idx) => (
                    <LineItem key={idx} code={item.code} description={item.description} value={item.value} />
                  ))}

                  {/* Current Liabilities */}
                  <AccSectionHeader title="Creditors: amounts falling due within one year" total={-(bs?.current_liabilities?.total || 0)} section="current_liabilities" icon={CreditCard} />
                  ...
                  <AccSectionHeader title="Capital and Reserves" total={bs?.capital_and_reserves?.total || 0} section="capital" icon={PiggyBank} />
