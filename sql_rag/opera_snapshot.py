"""
Opera Database Snapshot Utility

Takes snapshots of Opera tables before and after an operation
to understand exactly how Opera updates the database.

Usage:
    1. Run take_snapshot('before')
    2. Perform operation in Opera
...
CASHBOOK_TABLES = [
    ('aentry', 'ae_entry', 'ae_recdate DESC, ae_entry DESC'),  # Cashbook entries (header)
    ('abatch', 'ab_entry', 'ab_entry DESC'),                    # Batch header
    ('atran', 'at_entry', 'at_pstdate DESC, at_entry DESC'),   # Cashbook transactions
    ('stran', 'st_unique', 'st_trdate DESC, st_unique DESC'),  # Sales ledger transactions
    ('ptran', 'pt_unique', 'pt_trdate DESC, pt_unique DESC'),  # Purchase ledger transactions
    ('ntran', 'id', 'id DESC'),                                 # Nominal ledger transactions
    ('salloc', 'al_unique', 'al_date DESC, al_unique DESC'),   # Sales allocations
    ('palloc', 'al_unique', 'al_date DESC, al_unique DESC'),   # Purchase allocations
    ('sname', 'sn_account', 'sn_account'),                      # Customer balances
    ('pname', 'pn_account', 'pn_account'),                      # Supplier balances
    ('anoml', 'id', 'id DESC'),                                 # Cashbook transfer file
]
...
def take_snapshot(name: str, limit: int = 50, tables: Optional[List] = None) -> str:
    """
    Take a snapshot of Opera tables.
    ...
    for table_info in tables_to_snap:
        ...
        try:
            # Get column names
            cols_df = connector.execute_query(f"""
                SELECT COLUMN_NAME
...
def compare_snapshots(before_path: Optional[str] = None, after_path: Optional[str] = None) -> dict:
    ...
    for table_name in after['tables']:
        ...
        # Get primary key field (first in tuple)
        pk_field = None
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python opera_snapshot.py before   - Take 'before' snapshot")
        print("  python opera_snapshot.py after    - Take 'after' snapshot")
        print("  python opera_snapshot.py compare  - Compare latest snapshots")
        print("  python opera_snapshot.py list     - List all snapshots")
        sys.exit(1)
