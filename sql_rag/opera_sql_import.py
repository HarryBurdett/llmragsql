...
class OperaSQLImport:
    ...
    def import_nominal_entry(
        ...
        try:
            ...
            if nominal_check.empty:
                ...
                # Apply defaults if no code provided — Opera values: 1=Do Not Use, 2=Optional, 3=Mandatory
    ...
    def import_purchase_invoice_posting(
        ...
        try:
            ...
            def _do_purchase_invoice_posting(conn):
                ...
                self.update_nacnt_balance(conn, nominal_account, net_amount, period, year)
    ...
    def import_gocardless_batch(
        ...
        try:
            ...
            def _do_gocardless_batch(conn):
                ...
                if gocardless_fees > 0 and fees_nominal_account:
                    ...
                    if posting_decision.post_to_nominal:
                        ...
                        self.update_nacnt_balance(conn, fees_nominal_account, net_fees, period, year)
                        ...
                        if vat_on_fees > 0:
                            ...
                            self.update_nacnt_balance(conn, vat_nominal_account, abs(vat_on_fees), period, year)
                ...
                result_data['vat_nominal_used'] = vat_nominal_used
                result_data['vat_code_used'] = vat_code_used
            ...
            vat_nominal_used = result_data.get('vat_nominal_used')
            vat_code_used = result_data.get('vat_code_used', '2')
        ...
        try:
            ...
            def _do_complete_batch(conn):
                ...
                for _, anoml_row in anoml_df.iterrows():
                    ...
                    self.update_nacnt_balance(conn, nacnt_code, ax_value, period, year)
            ...
            def _do_recurring_post(conn):
                ...
                for idx, ln in enumerate(parsed_lines):
                    ...
                    if posting_decision.post_to_nominal:
                        ...
                        """))
                        self.update_nacnt_balance(conn, target_account, target_ntran_value, period, year)
