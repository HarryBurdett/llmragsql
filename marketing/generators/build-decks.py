...
JS = """
  (function () {
    ...
    function speakNow(script, myIdx) {
      ...
      // Only pin a voice when we actually resolved one: Safari can refuse an
      // utterance whose voice came from a stale getVoices() list.
      ...
      // Watchdog: if nothing is speaking shortly after we asked, fall back to
      // the timed advance so a silent failure cannot park the deck for good.
      setTimeout(function () {
        if (!playing || myIdx !== idx || started) { return; }
        var live = false;
        try { live = window.speechSynthesis.speaking || window.speechSynthesis.pending; } catch (err) { live = false; }
        if (!live) { queueAdvance(SILENT_DWELL); }
      }, 600);
      render();
    }
    ...
    function playCurrent() {
      ...
      if (muted || !speech || !script) { stopSpeech(); queueAdvance(SILENT_DWELL); render(); return; }

      // Safari drops an utterance when cancel() and speak() land in the same
      // tick, so when we are genuinely interrupting speech, cancel and let the
      // queue drain before speaking. On the first play there is nothing to
      // cancel, and we must speak synchronously to stay inside the user
      // gesture that authorises audio — deferring there would break Safari the
      // other way.
      var wasSpeaking = false;
      try { wasSpeaking = window.speechSynthesis.speaking || window.speechSynthesis.pending; } catch (err) { wasSpeaking = false; }
      stopSpeech();
      var myIdx = idx;
      if (wasSpeaking) { setTimeout(function () { speakNow(script, myIdx); }, 80); render(); }
      else { speakNow(script, myIdx); }
    }
