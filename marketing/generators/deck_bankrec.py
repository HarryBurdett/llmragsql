...
OUT OF SCOPE — do not reintroduce: foreign currency. bank-reconcile handles
HOME-CURRENCY bank accounts only (Harry, 2026-08-20). An earlier draft of this
deck claimed foreign-currency banks were "handled in their own currency", which
was wrong: FC support is a guard that refuses to post, not a feature. The
requirements line says home-currency explicitly so a prospect cannot infer
otherwise.
...
def slides(a):
    ...
    return [
      ...
      <div class="grid-2">
        ...
        <div class="panel"><h3>Every bank, every company</h3><p>Multi-company throughout, with its own settings, cashbook types and defaults per bank account.</p></div>
      ...
      <p class="note" style="margin-top:1.4rem;">
        Requirements: Pegasus Opera 3 or Opera SE, and the home-currency bank accounts you already
        keep in the cashbook. Open-banking statement feeds are optional &mdash; files and email work
        without them.
      </p>""",
...
NARRATION = [
    ...
    "Nothing changes in Opera. We read your cashbook and ledgers and write the transactions you approve: no migration, no new chart of accounts, no change to how your team works. Multi-company throughout, with its own settings and defaults for every bank account. And if Opera is ever restored from backup, the app spots it and helps you get back in step.",
