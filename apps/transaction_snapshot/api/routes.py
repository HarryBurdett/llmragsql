...
OPERA3_PRESETS = [
    ...
    {'module': 'allocations', 'name': 'Sales Receipt with Auto-Allocate',
    ...
    {'module': 'reconciliation', 'name': 'Bank Reconciliation — Unreconcile',
     'description': 'Reverse a previously-reconciled batch in Opera (un-mark the entries). Per entry on aentry: ae_reclnum/ae_statln/ae_recbal/ae_frstat/ae_tostat/ae_tmpstat all reset to 0/blank. On nbank: nk_recbal and the last-rec fields WALK BACK to the prior batch\'s stamped state (or reset to fresh-bank values when no reconciled entries remain). Establishes the authoritative walk-back semantics for the agent\'s /reconcile/unmark. Capture immediately after a Mark Reconciled snapshot so the pair round-trips.'},
    {'module': 'gocardless', 'name': 'GoCardless Payout — Full Chain',
