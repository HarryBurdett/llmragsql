...
OPERA3_PRESETS = [
    ...
    {'module': 'gocardless', 'name': 'GoCardless Batch Import',
     'description': 'A GoCardless payout imported as a batch: one sales receipt per collected payment (at_type 4, allocated to its invoice — stran + salloc + sname), plus the GoCardless fee posted as an expense with input VAT (ntran + zvtran). Bank credited with the NET payout (nbank.nk_curbal). Shared batch entry numbering (atype.ay_entry). Bank reference written to ae_entref for later matching.'},
