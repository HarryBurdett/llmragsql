...
## Application Architecture (Updated 2026-03-24)

### Modular Structure

The monolithic `api/main.py` has been split into independent app modules:

```
apps/
  core/state.py              → shared globals (sql_connector, email_storage, config, etc.)
  bank_reconcile/api/routes.py  → 117 endpoints (bank import, reconcile, cashbook)
  gocardless/api/routes.py      → 120 endpoints (import, mandates, subscriptions, requests)
  balance_check/api/routes.py   → 14 endpoints (debtors, creditors, trial balance, VAT)
  dashboards/api/routes.py      → 33 endpoints (CEO KPIs, revenue, credit control)
  suppliers/api/routes.py       → 34 endpoints (statements, queries, communications)
  lock_monitor/api/routes.py    → 25 endpoints (SQL Server + Opera 3 lock monitoring)
  pension_export/api/routes.py  → 15 endpoints (pension schemes, NEST, contributions)
```

### Per-Request Globals Sync

Extracted route modules use bare names (`sql_connector`, `email_storage`, etc.) that are populated per-request by `_sync_from_main()`, called from `_ensure_company_context()` in `api/main.py`. Single-threaded async (one uvicorn worker) makes this safe.

Each route module declares:
```python
_SYNC_NAMES = ('sql_connector', 'email_storage', 'config', ...)
...
**Critical**: Any function defined in `api/main.py` that is used as a bare name in route modules must be added to `_SYNC_NAMES`. Module-level `__getattr__` (PEP 562) does NOT work for bare names inside function bodies — only for external attribute access.

### BankStatementImport Connection

`BankStatementImport` must receive the per-request `sql_connector` via its `sql_connector` parameter:
```python
importer = BankStatementImport(bank_code=bank_code, sql_connector=sql_connector)
```
Without this, it creates a new `SQLConnector()` from `config.ini` which may fail if the ODBC driver or connection settings differ from the running app's context.

---

## Multi-Company Design

### Company Context Per Request

Every request uses the logged-in company's settings. The `_ensure_company_context(company_id)` middleware:
1. Swaps `sql_connector` to the company's SQL connection
2. Swaps `email_storage` to the company's email database
3. Re-registers email providers for the company
4. Updates `customer_linker` connector
5. Resets import lock paths, supplier DB, extraction cache, payments DB
6. Syncs globals to all extracted route modules

### Company-Scoped Resources

| Resource | Storage | Keyed By |
|----------|---------|----------|
| SQL connector | `_company_sql_connectors[company_id]` | company_id |
| Email storage | `_company_email_storages[company_id]` | company_id |
| Email sync | `_company_email_sync_managers[company_id]` | company_id |
| Bank patterns | `data/{company_id}/bank_patterns.db` | company_id |
| Import locks | `data/{company_id}/import_locks.db` | company_id |
| GC payments | `data/{company_id}/gocardless_payments.db` | company_id |
| Supplier statements | `data/{company_id}/supplier_statements.db` | company_id |
| GC settings | `data/{company_id}/gocardless_settings.json` | company_id |

### Absolute Rules
- **NEVER hardcode** account codes, bank names, company IDs, or skip patterns
- Bank lock keys are company-scoped: `f"{company_id}:{bank_code}"`
- Control accounts loaded dynamically from Opera configuration per company

---

## Bank Statement Processing (Updated 2026-03-24)

### Statement Validation
A statement is valid for import when ALL conditions are met:
1. **Account match**: Sort code AND account number match the selected Opera bank (`nbank.nk_sort`, `nbank.nk_number`)
2. **Balance match**: Opening balance equals Opera's reconciled balance (`nbank.nk_recbal / 100.0`) within £0.01
3. **Sequential import**: Statements must be imported in order

### Opening Balance
- **Authoritative source**: Opera's reconciled balance (`nk_recbal`) — NOT the PDF-extracted value
- AI extraction may pick up wrong account's balance (e.g., savings vs current)
- If mismatch: log warning, use reconciled balance, update extraction cache

### Closing Balance
- Validated via **balance chain**: walks from opening balance through each transaction where `current + amount = next_balance`
- Excludes phantom transactions from wrong accounts
- Chain validation catches AI extraction errors

### Amount Parsing
- Always use `_safe_float()` / `_parse_amount()` — strips commas, currency symbols
- Gemini returns formatted strings like "1,770.00" which crash bare `float()`

### Archive Not Delete
- Statement files moved to `archive/YYYY-MM/` folder
- Emails moved to IMAP `Archive/Bank Statements`
- User can restore from Archived Statements section
- Managed filenames checked by base name (without counter suffix `_37`, `_38`)
...
Opera repeat invoices (`ih_docstat = 'U'`) use `ih_ignore` for frequency:

| Code | Description | GoCardless Interval |
|------|-------------|-------------------|
| W | Weekly | weekly, 1 |
| F | Fortnightly | weekly, 2 |
| M | Monthly | monthly, 1 |
| B | Bi-monthly | monthly, 2 |
| Q | Quarterly | monthly, 3 |
| H | Half-yearly | monthly, 6 |
| A | Annual | yearly, 1 |
| D | Every N days | custom (ih_dcontr = days between) |

### Repeat Invoice Fields (ihead)
- `ih_doc` — Document reference
- `ih_account` — Customer account
- `ih_name` — Customer name
- `ih_ignore` — Frequency code (W/F/M/B/Q/H/A/D)
- `ih_dcontr` — Days between (for D frequency)
- `ih_scontr` — Contract start date
- `ih_econtr` — Contract end date
- `ih_job` — Department code
- `ih_analsys` — Analysis code (used for SUB tagging)
- `ih_custref` — Customer reference
- `ih_docstat` — Document status ('U' = active repeat)

---

## Opera Column Name Corrections

| Wrong name | Correct name | Table | Notes |
|---|---|---|---|
| `at_date` | `at_pstdate` | atran | Post date |
...
## Opera Period Status Values

| Value | Status | Can Post? |
|-------|--------|-----------|
| 0 | Open | Yes |
| 1 | Blocked | No |
| 2 | Closed | No |
